...
from account.models import StaffUserProfile, User

from .models import Product, ProductSale, Sale, Store
...
class StoreSerializer(serializers.ModelSerializer):
    ...
    def get_today_total(self, obj):
        ...
        today_sales = ProductSale.objects.filter(
            sale__created_at__date=today, sale__store=obj
        )
        total_sales_today = (
    ...
    def get_week_total(self, obj):
        ...
        week_sales = ProductSale.objects.filter(
            sale__created_at__date__range=[start_of_week, end_of_week], sale__store=obj
        )
        total_sales_week = (
            week_sales.aggregate(
                total=Sum(F("quantity") * F("product__selling_price"))
            )["total"]
            or 0
        )
    ...
    def get_staff_id(self, obj):
        try:
            return (
                StaffUserProfile.objects.filter(store=obj).first().id
...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        made_by_user_id = validated_data[
            "made_by"
        ]  # the user who made the sale (might be the admin, or one of the staff. Multiple staff will be possible in the future)
        ...
        for sale in sales:
            ...
            if product.amount_in_stock < int(sale["quantity"]):
                ...
                raise serializers.ValidationError(
                    f"Not enough stock for product {product.name} ({product.amount_in_stock} in stock)"
                )
            product_sale = ProductSale.objects.create(
                product=product,
                quantity=int(sale["quantity"]),
                previous_quantity=product.amount_in_stock,
                sale=new_sale,
            )
            sale_total += int(sale["quantity"]) * product.selling_price
